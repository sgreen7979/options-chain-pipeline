...
def setSchwabClientClass(klass):
    ...
    global _schwabClientClass
    _schwabClientClass = klass
...
def getSchwabClientClass():
    ...
    """
    return _schwabClientClass


from .new import SchwabClient  # noqa

# _schwabClientClass = SchwabClient
...
# # from daily.schwab.accounts import get_credentials
# from daily.schwab.credentials.functions import get_credentials
# from daily.schwab.enums import VALID_CHART_VALUES
# from daily.schwab.exceptions import ExdLmtError
# from daily.schwab.exceptions import ForbidError
# from daily.schwab.exceptions import GeneralError
# from daily.schwab.exceptions import NotFndError
# from daily.schwab.exceptions import NotNulError
# from daily.schwab.exceptions import ServerError
# from daily.schwab.exceptions import TknExpError
# from daily.schwab.option_chain import OptionChain as OptionChainParams
# from daily.schwab.orders import Order
# from daily.utils.logging import get_logger

# if TYPE_CHECKING:
#     from daily.schwab.credentials import SchwabCredentials
#     from daily.schwab.accounts import Account


# logger = get_logger("SchwabClient", fh=True, fh_level=logging.DEBUG)
...
#     def _purge_request_times(self) -> None: ...
...
#     def _purge_request_times(self: CapacityLimiterProto) -> None:
#         """Purge timestamps from the request times list that are older
#         than 60 seconds
#         """
#         with self.lock:
...
#     def get_request_times(self: CapacityLimiterProto) -> List[float]:
#         """Returns a list of timestamps of when requests were made within
#         the last 60 seconds
#         """
#         self._purge_request_times()
#         with self.lock:
#             req_times = self.request_times.copy()  # copy to avoid race conditions
#         return req_times

#     def get_capacity(self: CapacityLimiterProto) -> int:
#         """Returns the number of requests the client can accomodate"""
#         with self.lock:
#             self._purge_request_times()
#             return self.config["rolling_sixty_limit"] - len(self.request_times)

#     def has_capacity(self: CapacityLimiterProto) -> bool:
#         """Returns True if client has capacity else False"""
#         return self.get_capacity() > 0

#     def get_next_capacity_time(self: CapacityLimiterProto) -> float:
#         """Returns the number of seconds until client will have capacity"""
#         if self.has_capacity():
#             return 0.0
#         else:
#             earliest_req_time = self.get_request_times()[0]
#             return earliest_req_time + 60.0 - time.time()

#     def get_next_capacity_dt(self: CapacityLimiterProto) -> dt.datetime:
#         """Returns the number of seconds until client will have capacity"""
#         if self.has_capacity():
#             return dt.datetime.now()
#         else:
#             earliest_req_dt = dt.datetime.fromtimestamp(self.get_request_times()[0])
#             return earliest_req_dt + dt.timedelta(seconds=60.0)

#     def get_capacity_at(
#         self: CapacityLimiterProto,
...
#             ts = t
#         cutoff = ts - 60.0
#         req_times = [rt for rt in self.get_request_times() if rt >= cutoff]
#         if future_req_times is not None:
#             future_req_ts = [
#                 irt.timestamp() if isinstance(irt, dt.datetime) else irt
#                 for irt in future_req_times
#             ]
#             future_req_ts = sorted([irt for irt in future_req_ts if irt >= cutoff])
#             req_times.extend(future_req_ts)
#         return self.config["rolling_sixty_limit"] - len(req_times)


# class SchwabClient(CapacityLimiterMixin):
...
#             self.request_times.extend([ts for _ in range(multi)])
#         else:
...
#             raise NotNulError(message=message)
#         elif response.status_code == 401:
#             raise TknExpError(message=message)
#         elif response.status_code == 403:
#             raise ForbidError(message=message)
#         elif response.status_code == 404:
#             raise NotFndError(message=message)
#         elif response.status_code == 429:
#             raise ExdLmtError(message=message)
#         elif response.status_code in (500, 503):
#             raise ServerError(message=message)
#         elif response.status_code > 400:
#             raise GeneralError(message=message)
#         else:
#             raise GeneralError(message=message)
...
#         # return the response of the get request.
#         return self._make_request(
#             method="get", endpoint=endpoint, params=params, incl_fetch_time=True
#         )
...
#         return cls(
#             account.api_key, account.secret, account.redirect_uri, account.token_path
#         )

#     @classmethod
#     def from_credentials(cls, credentials: "SchwabCredentials"):
#         return cls(
#             credentials.api_key, credentials.secret, credentials.redirect_uri, credentials.token_path
#         )

#     @classmethod
#     def from_account_idx(cls, idx: int):
#         """
