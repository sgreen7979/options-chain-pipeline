...
class OptionChain:
    ...
    def __init__(
        ...
        Arguments:
        ----
        symbol (str): A single symbol to return option chains for,
        ...
        from_date str, optional): Only return expirations after this date.
            For strategies, expiration refers to the nearest term expiration in the strategy.
            Valid ISO-8601 formats are: yyyy-MM-dd. Defaults to None.

        to_date (str, optional): Only return expirations before this date.
            For strategies, expiration refers to the nearest term expiration in the strategy.
            Valid ISO-8601 formats are: yyyy-MM-dd. Defaults to None.
        ...
        exp_month (str, optional): Return only options expiring in the specified month.
        ...
        entitlement (str) Applicable only if its retail token, entitlement of client PP-PayingPro,
        NP-NonPro and PN-NonPayingPro. vailable values : PN, NP, PP
        ...
        strategy = OptionChain.Strategy.SINGLE if not strategy else strategy.upper()
        opt_range = OptionChain.Range.ALL if not opt_range else opt_range.upper()
        ...
        self.query_parameters = {
            ...
            "includeUnderlyingQuote": include_quotes,
            "strategy": strategy,
            "interval": interval,
            "strike": strike,
            "range": opt_range,
            ...
            "entitlement": entitlement,  # SCHWAB Applicable only if its retail token, entitlement of client PP-PayingPro, NP-NonPro and PN-NonPayingPro
        }
    ...
    def validate_chain(self) -> bool:
        ...
        if (
            "strategy" in self.query_parameters.keys()
            and self.query_parameters["strategy"] == "SINGLE"
        ):
        ...
        for key, value in self.query_parameters.items():
            ...
            if key in self.argument_types.keys() and (
                value not in self.argument_types[key]
            ):
                raise KeyError(
                    incorrect_val_msg.format(
                        val=value, key=key, corr=", ".join(self.argument_types[key])
                    )
                )
    ...
    def add_chain_key(
        self, key_name: Optional[str] = None, key_value: Optional[str] = None
    ) -> None:
        ...
        if key_name not in self.query_parameters:
            raise KeyError(
                incorrect_key_msg.format(
                    incorrect_val=key_name, correct_val=correct_vals
                )
            )

        # If possible, validate that the value can be used.
        if (
            key_name in self.argument_types.keys()
            and key_value not in self.argument_types[key_name]
        ):
            raise ValueError(
                incorrect_val_msg.format(
