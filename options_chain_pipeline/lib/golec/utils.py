    ...
    def type_error(self, value, required_enum_type):
        ...
        if isinstance(value, str):
            ...
            if possible_members:
                possible_members_message = (
                    'Did you mean '
                    + ', '.join(
                        possible_members[:-2] + [' or '.join(possible_members[-2:])]
                    )
                    + '? '
                )
        ...
        raise ValueError(
            (
                ...
                + 'enforce_enums=False to disable this checking)'
            ).format(
...
class Utils(EnumEnforcer):
    ...
    def extract_order_id(self, place_order_response):
        ...
        m = re.match(
            r'https://api.schwabapi.com/trader/v1/accounts/(\w+)/orders/(\d+)', location
        )
