...
class StockFrame:
    ...
    def do_indicator_exist(self, column_names: List[str]) -> bool:
        ...
        if set(column_names).issubset(self._frame.columns):
            ...
            raise KeyError(
                "The following indicator columns are missing from the StockFrame: {missing_columns}".format(
    ...
    def _check_signals(
        ...
        if self.do_indicator_exist(column_names=indicators_key):
            for indicator in indicators_key:
                ...
                condition_1 = condition_1.where(lambda x: x is True).dropna()
                condition_2 = condition_2.where(lambda x: x is True).dropna()
