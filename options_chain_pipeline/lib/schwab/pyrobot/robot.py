# Adapted from https://github.com/areed1192/python-trading-robot by Alex Reed
...
milliseconds_since_epoch = SchwabUtilities().milliseconds_since_epoch
...
class PyRobot:
    def __init__(
        ...
        trading_account {str} -- Your Broker account number. (default: {None})
        ...
        self.session: SchwabClient = self._create_session()
        self.trades = {}
    ...
    def _create_session(self) -> SchwabClient:
        """Start a new session.

        Creates a new session with the Broker API and logs the user into
        the new session.

        Returns:
        ----
        Broker -- A Broker object with an authenticated sessions.
    ...
    def create_trade(
        ...
        # Set the Client.
        assert isinstance(self.trading_account, str)
    ...
    def grab_current_quotes(self) -> dict:
        ...
        Makes a call to the Broker Get Quotes endpoint with all
    ...
    def grab_historical_prices(
        ...
        symbols: Optional[Union[List[str], Dict[str, Any]]] = None,
        bar_size: int = 1,
        bar_type: str = "minute",
    ) -> dict:
        ...
        start_str = str(milliseconds_since_epoch(dt_object=start))
        end_str = str(milliseconds_since_epoch(dt_object=end))
    ...
    def wait_till_next_bar(self, lastbar_timestamp: pd.DatetimeIndex) -> None:
        ...
        last_bar_time = lastbar_timestamp.to_pydatetime()[0].replace(
            tzinfo=timezone.utc
        )
    ...
    def get_accounts(
        self, account_number: Optional[str] = None, all_accounts: bool = False
    ) -> dict:
