...
class StatePath(type(pathlib.Path())):
    def __init__(self, credentials_file: Optional[str] = None):
        ...
        self.credenitals_file_name = "schwab_credentials.json"
    ...
    def settings_directory(self) -> pathlib.PurePath:
        """Returns the `.schwab_python_library` directory path.
    ...
    def path_library(self) -> pathlib.PurePath:
        """Generates the Schwab Library Path.

        Returns:
        ----
        {pathlib.PurePath} -- A PurePath object pointing to the Schwab
            library.
        """
    ...
    def path_settings(self) -> pathlib.PurePath:
        """Generates a path to the `.schwab_python_library` directory.

        Returns:
        ----
        {pathlib.PurePath} -- A PurePath object pointing to the `.schwab_python_library`
            directory.
        """
        self.home_directory
        settings_directory = self.home_directory.joinpath(".schwab_python_library")
        return settings_directory

    def json_settings_path(self):
        """Generates a path to the `.schwab_python_library/schwab_credentials.json` file.

        Returns:
        ----
        {pathlib.PurePath} -- A PurePath object pointing to the
             `.schwab_python_library/schwab_credentials.json` file.
        """
    ...
    def write_credentials(
        self, file_path: Union[pathlib.Path, str], state: dict
    ) -> pathlib.Path:
        """Writes the credentials to the Settigns folder.
...
class SchwabUtilities:
    ...
    def datetime_from_milliseconds_since_epoch(
        self, ms_since_epoch: int, timezone: datetime.timezone = None
    ) -> datetime.datetime:
