...
def __fetch_and_register_token_from_redirect(
    ...
    update_token = (
        __update_token(token_path) if token_write_func is None else token_write_func
    )
    ...
    if asyncio:
        ...
        oauth_client_update_token = update_token  # type: ignore
...
def client_from_token_file(
    token_path, api_key, app_secret, asyncio=False, enforce_enums=True
):
    '''
...
def client_from_manual_flow(
    ...
    '''
    get_logger().info(
        'Creating new token with callback URL \'%s\' ' + 'and token path \'%s\'',
        callback_url,
        token_path,
    )
    ...
    if callback_url.startswith('http://'):
        print(
            (
                'WARNING: Your callback URL ({}) will transmit data over HTTP, '
...
def client_from_access_functions(
    ...
    if asyncio:
        ...
        oauth_client_update_token = wrapped_token_write_func  # type: ignore
        session_class = OAuth2Client
        client_class = Client
...
def easy_client(
    ...
    *Reminder:* You should never create the token file yourself or modify it in
    ...
    if c is None:
        ...
        logger.info(
            'Returning client fetched using web browser, writing' + 'token to \'%s\'',
            token_path,
        )
