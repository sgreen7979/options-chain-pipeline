...
from ..auth import client_from_token_file
from ..contrib.orders import construct_repeat_order, code_for_builder
...
def latest_order_main(sys_args):
    ...
    required.add_argument('--token_file', required=True, help='Path to token file')
    ...
    account_spec_group.add_argument(
        '--account_id', type=int, help='Restrict lookups to a specific account ID'
    )
    ...
    client = client_from_token_file(args.token_file, args.app_secret, args.api_key)
    ...
    if args.account_id is not None:
        ...
        for val in r.json():
            ...
            print(
                (
                    'Failed to find account has for account ID {}. Searched '
                    + 'the following accounts:\n{}'
                ).format(args.account_id, json.dumps(r.json(), indent=4))
            )
            return -1
    else:
    ...
    def get_orders(method):
        ...
        if r.status_code != httpx.codes.OK:
            print(
                (
                    'Returned HTTP status code {}. This is most often caused '
                    + 'by an invalid account ID or hash.'
    ...
    if account_hash is not None:
        ...
        if 'error' in orders:
            print(
                (
                    'Schwab returned error: "{}", This is most often caused '
                    + 'by an invalid account ID or hash'
                ).format(orders['error'])
            )
            return -1
    ...
    if orders:
        ...
        if 'requestedDestination' in order and order['requestedDestination'] != 'AUTO':
            print(
                (
                    '# Warning: This order contains a non-"AUTO" value of '
                    + '"requestedDestination" ("{}").'
                ).format(order['requestedDestination'])
            )
            print(
                '#          This parameter appears to be broken in '
                + 'the API, so it is omitted in this generated code.'
            )
            emit_destination_warning_newline = True
        if (
            'destinationLinkName' in order
            and order['destinationLinkName'] != 'AutoRoute'
        ):
            print(
                (
                    '# Warning: This order contains a non-"AutoRoute" value of '
                    + '"destinationLinkName" ("{}").'
                ).format(order['destinationLinkName'])
            )
            print(
                '           This parameter appears to be broken in the '
