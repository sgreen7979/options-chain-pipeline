...
_FIELDS_AND_SETTERS = (
    ...
    ('priceLinkBasis', 'set_price_link_basis', schwab.orders.common.PriceLinkBasis),
    ('priceLinkType', 'set_price_link_type', schwab.orders.common.PriceLinkType),
    ...
    if 'orderStrategyType' in historical_order:
        if historical_order['orderStrategyType'] == 'TRIGGER':
            builder = schwab.orders.common.first_triggers_second(
                builder,
                construct_repeat_order(historical_order['childOrderStrategies'][0]),
            )
        elif historical_order['orderStrategyType'] == 'OCO':
            builder = schwab.orders.common.one_cancels_other(
                construct_repeat_order(historical_order['childOrderStrategies'][0]),
                construct_repeat_order(historical_order['childOrderStrategies'][1]),
            )
    else:
    ...
    if 'orderLegCollection' in historical_order:
        for leg in historical_order['orderLegCollection']:
            if leg['orderLegType'] == 'EQUITY':
                ...
                raise ValueError('unknown orderLegType {}'.format(leg['orderLegType']))
...
class EquityOrderLegAST:
    ...
    def render(self, imports, lines, paren_depth=0):
        ...
        lines.append(
            '.add_equity_leg(EquityInstruction.{}, "{}", {})'.format(
...
class OptionOrderLegAST:
    ...
    def render(self, imports, lines, paren_depth=0):
        ...
        lines.append(
            '.add_option_leg(OptionInstruction.{}, "{}", {})'.format(
                self.instruction, self.symbol, self.quantity
            )
        )
    ...
    def __init__(self, builder):
        ...
        for leg in builder._orderLegCollection:
            if leg['instrument']._assetType == 'EQUITY':
                self.top_level_fields.append(
                    EquityOrderLegAST(
                        leg['instruction'], leg['instrument']._symbol, leg['quantity']
                    )
                )
                ...
                raise ValueError(
                    'unknown leg asset type {}'.format(leg['instrument']._assetType)
                )
